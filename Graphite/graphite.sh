...
rpm -qa | grep epel
if [ $? -ne 0 ]; then
	echo "WARNING: epel-release is not installed. Correct this by running:"
...
# Grab dependencies for building the RPMs and supervisord
yum -y install rpm-build ruby-devel gcc rubygems supervisor

# Install fpm gem for more building
gem install fpm

# Build carbon via fpm
fpm -s python -t rpm -d python-setuptools -d Django -d django-tagging -d python-devel -d python-twisted -d python-memcached -d python-sqlite2 -d bitmap -d bitmap-console-fonts -d bitmap-fixed-fonts -d bitmap-fonts-compat -d bitmap-lucida-typewriter-fonts -d bitmap-miscfixed-fonts -d pycairo carbon

# Build whisper via fpm
fpm -s python -t rpm -d python-setuptools -d Django -d django-tagging -d python-devel -d python-twisted -d python-memcached -d python-sqlite2 -d bitmap -d bitmap-console-fonts -d bitmap-fixed-fonts -d bitmap-fonts-compat -d bitmap-lucida-typewriter-fonts -d bitmap-miscfixed-fonts -d pycairo whisper

# Build graphite-web via fpm
fpm -s python -t rpm -d python-setuptools -d Django -d django-tagging -d python-devel -d python-twisted -d python-memcached -d python-sqlite2 -d bitmap -d bitmap-console-fonts -d bitmap-fixed-fonts -d bitmap-fonts-compat -d bitmap-lucida-typewriter-fonts -d bitmap-miscfixed-fonts -d pycairo -d python-gunicorn graphite-web

# Install the newly created RPMs with required deps
...
cp carbon.conf /opt/graphite/conf/
...
echo "Install complete. Reboot to activate Graphite and connect on port 8080 to test."
